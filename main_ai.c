...
/*
 * Moving to windows years later, below guards is detected and compiles in CLion
 * However it doesn't acutally run, vars used in it reported unused by analysis
 * Identical code in main_omp completely ignored
 */
...
// since position of nodes doesn't change (but location of children array can) be sure to copy then unlock, then follow and lock next node
struct tree_node
{
...
// only run in locked section, so relatively safe
unsigned int
...
// locked in here to make code more legible
void
...
run_mcts(binmatrix* black_in, binmatrix* white_in, binmatrix* old_black_in, binmatrix* old_white_in, enum side ai_side)
{
	...
	while (cur->num_children != 0 && cur->num_children >= (0.75*free_spots) && free_spots >= 20)
	{
		...
		index = ucb1_search(cur);
		next = cur->children[index];
		cur->success--; // virtual loss
...
//# endif
		...
		if (turn == winner)
			cur->success++;
		cur->success++; // make up for virtual loss
		cur->hits++;
