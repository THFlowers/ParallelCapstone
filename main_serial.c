...
main(int argc, char* argv[])
{
	...
	running_leaves = malloc(sizeof(struct tree_node)*num_procs);
	root = tree_node_new();
...
run_mcts(binmatrix* black_in, binmatrix* white_in, binmatrix* old_black_in, binmatrix* old_white_in, enum side ai_side)
{
	...
	// pick new move
	struct point move;
	enum side new_node_side = turn; // save side of current turn for backprop
	int flag = 0;
	...
	struct tree_node* new_leaf = tree_node_new();
...
/* Original random_simulation saved for posterity, necessity made it ugly
void
random_simulation(
	...
	enum side start_turn)
{
	...
	for (unsigned int num_moves=0; num_moves < max_moves; num_moves++)
	{
		...
		} while (FALSE == play_move(black, white, old_black, old_white, blk_score, wht_score, move, &cur_turn));
	}
}
*/
